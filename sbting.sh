#!/bin/bash
# Sing-box 静默部署脚本 (自签证书 + VLESS TLS + HY2 UDP+QUIC)
# Author: ChatGPT
...
# 安装依赖
apt update -qq
apt install -y -qq curl socat openssl qrencode dnsutils

# 安装 sing-box
if ! command -v sing-box &>/dev/null; then
    bash <(curl -fsSL https://sing-box.app/deb-install.sh) &>/dev/null
fi
...
# 检查端口
[[ -n "$(ss -tulnp | grep $VLESS_PORT)" ]] && echo "[✔] VLESS TCP $VLESS_PORT 已监听"
[[ -n "$(ss -ulnp | grep $HY2_PORT)" ]] && echo "[✔] HY2 UDP/QUIC $HY2_PORT 已监听"
