...
mkdir -p "$CONFIG_DIR" "$CERT_DIR" /usr/local/bin
...
echo "[*] 安装依赖..."
apk update
apk add --no-cache bash curl wget socat openssl iproute2 dcron || true
...
# 读取端口/模式/域名
[ -f "$PORT_FILE" ] || echo "0 0" > "$PORT_FILE"
